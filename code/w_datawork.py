...
# def draw_price_by_prodercs(df: pd.DataFrame, plot_name: str) -> None:
#     pass


# # Загружаем данные от парсера и после чистим их
# 

# mask = milk['Товар'] != 'Молоко'
# milk.loc[mask, ['Товар', 'Поставщик']] = milk.loc[mask, ['Поставщик', 'Товар']].values

# # теперь ищем минимальную цену для каждого производителя, рейтинг берём как среднее, считая, что товары одного производителя +- одинаковы по качеству
# milk_minprice_producers = milk.loc[milk.groupby('Поставщик')['Цена за кг/л'].idxmin()].copy()
# milk_minprice_producers['Рейтинг'] = milk.groupby('Поставщик')['Рейтинг'].transform('mean').round(1)
# milk_minprice_producers = milk_minprice_producers.sort_values('Цена за кг/л').reset_index(drop=True)


# # рисуем и сохраняем график производитель — минимальная цена

# fig, ax = plt.subplots(figsize=(14, 8))

# ax.scatter(milk_minprice_producers['Цена за кг/л'], 
#            milk_minprice_producers['Рейтинг'],
#            s=100, alpha=0.7, c='steelblue', edgecolors='white', linewidth=1.5)

# # Смещённые номера вместо названий
# offsets = [(5, 5), (-10, 5), (5, -10), (-10, -10), (10, -5), (-5, 10)]
# for i, (_, row) in enumerate(milk_minprice_producers.iterrows()):
#     offset = offsets[i % len(offsets)]
#     ax.annotate(str(i+1), 
#                 (row['Цена за кг/л'], row['Рейтинг']),
#                 fontsize=9, fontweight='bold', 
#                 xytext=offset, textcoords='offset points')

# ax.set_xlabel('Цена за кг/л (руб.)', fontsize=10)
# ax.set_ylabel('Рейтинг', fontsize=10)
# ax.set_title('Поставщики молока: цена vs рейтинг', fontsize=12)

# # Таблица с соответствием номеров и названий
# table_data = [[f"{i+1}. {row['Поставщик']}"] for i, (_, row) in enumerate(milk_minprice_producers.iterrows())]
# table = ax.table(cellText=table_data, loc='right', bbox=[1.15, 0, 0.3, 1])
# table.auto_set_font_size(False)
# table.set_fontsize(8)
...
# # Нормализация (0-1) с инверсией для цены (чем ниже цена, тем выше скор)
# milk_minprice_producers['цена_норм'] = 1 - (milk_minprice_producers['Цена за кг/л'] - milk_minprice_producers['Цена за кг/л'].min()) / (milk_minprice_producers['Цена за кг/л'].max() - milk_minprice_producers['Цена за кг/л'].min())
# milk_minprice_producers['рейтинг_норм'] = (milk_minprice_producers['Рейтинг'] - milk_minprice_producers['Рейтинг'].min()) / (milk_minprice_producers['Рейтинг'].max() - milk_minprice_producers['Рейтинг'].min())

# # Скоринг: рейтинг (вес 0.7) + цена (вес 0.3)
# milk_minprice_producers['Скор'] = (milk_minprice_producers['рейтинг_норм'] * 0.7 + 
#                                      milk_minprice_producers['цена_норм'] * 0.3).round(3)

# # Сортировка по убыванию скора
# milk_minprice_producers = milk_minprice_producers.sort_values('Скор', ascending=False).reset_index(drop=True)

# # Столбчатая диаграмма
# fig, ax = plt.subplots(figsize=(14, 8))

# colors = plt.cm.RdYlGn(milk_minprice_producers['Скор'] / milk_minprice_producers['Скор'].max())
# bars = ax.barh(range(len(milk_minprice_producers)), milk_minprice_producers['Скор'], color=colors)

# ax.set_yticks(range(len(milk_minprice_producers)))
# ax.set_yticklabels(milk_minprice_producers['Поставщик'], fontsize=9)
# ax.set_xlabel('Баллы', fontsize=10)
# ax.set_ylabel('Поставщик', fontsize=10)
# ax.set_title('Рейтинг поставщиков молока (0.7*Рейтинг + 0.3*ЦенаНормированная)', fontsize=12)
# ax.invert_yaxis()  # Чтобы лучший был сверху
